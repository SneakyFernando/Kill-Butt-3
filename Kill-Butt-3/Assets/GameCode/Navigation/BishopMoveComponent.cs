...
class BishopMoveComponent : MoveComponent
{
	protected override void Initialize()
	{
		...
		for(int i = 1; i < 9; i += 2)
		{
			for(int j = 1; j < 9; j++)
			{
				...
				Vector3 lastKey = Snap(Quaternion.AngleAxis(45 * i, Vector3.up) * Vector3.forward) * j;
				move.Push(lastKey);
	...
	public override void UpdateUnderAttack()
	{
		...
		for(int i = 1; i < 9; i += 2)
		{
			for(int j = 1; j < 9; j++)
			{
				Vector3 attackPointRel = Snap(Quaternion.AngleAxis(45 * i, Vector3.up) * Vector3.forward) * j;
